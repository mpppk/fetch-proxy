...
CORS 付きの汎用 fetch プロキシ (Cloudflare Workers + Hono)。任意の `https://` URL を `https://fetch.nibo.sh/<host>/<path>` 経由で取得し、`html` / `title` / `markdown` 形式で返却します。
...
| Method | Path | 説明 |
|--------|------|------|
| `GET` | `/` | `as` なしなら利用方法を返す `200 text/plain: "fetch-proxy: use /<host>/<path>?as=html\|title\|md"`。それ以外は `400` |
...
| 名前 | 必須 | 型 | デフォルト | 説明 |
|------|------|----|-----------|------|
| `as` | no | `html \| title \| md` | `html` | レスポンス形式。複数回指定すると `400 as parameter cannot be specified multiple times` |
...
| `400` | `invalid as value: foo. allowed: html, title, md` | 不正な `as` |
...
# 3. Markdown (defuddle → Browser fallback)
curl https://fetch.nibo.sh/example.com/articles/123?as=md
# => # Article Title
#    Article body...
...
* **title 抽出:** `meta[property="og:title"]` → `meta[name="og:title"]` → `<title>` の優先順位、失敗時は regex フォールバック (`src/index.ts:49`)
